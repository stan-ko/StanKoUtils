...
//    private boolean mDiskCacheStarting = true;
    ...
    public static ImageCache getInstance(Activity activity, ImageCacheParams cacheParams) {
        ...
        // No existing ImageCache, create one and store it in RetainFragment
        if (activityId == null || activity.getClass().getName()!=activityId || imageCache==null || imageCache.mReusableBitmaps==null || imageCache.mReusableBitmaps.size()==0) {
        	if (imageCache!=null) 
        		imageCache.clearCache();
            imageCache = new ImageCache(cacheParams);
           // mRetainFragment.setObject(imageCache);
            activityId = activity.getClass().getName();
        }
    ...
    /**
     * Return an App global {@link ImageCache} instance (not Activity's personal). 
     *
     * @param cacheParams The cache parameters to use if the ImageCache needs instantiation.
     * @return An existing retained ImageCache object or a new one if one did not exist
     */
    public static ImageCache getInstance(ImageCacheParams cacheParams) {
        // No existing ImageCache, create one and store
        if (imageCache==null )
            imageCache = new ImageCache(cacheParams);
...
//        public static final float boolean initDiskCacheOnCreate = DEFAULT_INIT_DISK_CACHE_ON_CREATE;
		 ...
		 * {@link ImageWorker#addImageCache(FragmentManager, ImageCacheParams)}.
		 * 
		 * @param float memCacheSizePercent [0.05;0.08] sets the memory cache
		 *        size based on a percentage of the max available VM memory.
		 */
        public ImageCacheParams(final float memCacheSizePercent) {
        	if (memCacheSizePercent>0)
        		setMemCacheSizePercent(memCacheSizePercent);
        }
...
//         *                               is sufficient.
//         */
//        public ImageCacheParams(final Context context, final float memCacheSizePercent, final boolean useDiskCache) {
//        	if (useDiskCache)
//        		diskCacheDir = SDCardHelper.getCacheDir(context);
//        	if (memCacheSizePercent>0)
//        		setMemCacheSizePercent(memCacheSizePercent);
//        }
//        public ImageCacheParams(String diskCacheDirectoryName) {
//        	diskCacheDir = new File(diskCacheDirectoryName);
//        }
//        public ImageCacheParams(File diskCacheDirectory) {
//        	diskCacheDir = diskCacheDirectory;
//        }
        ...
        public void setMemCacheSizePercent(final float percent) {
            if (percent < 0.05f || percent > 0.8f) {
            	Log.e(this, new IllegalArgumentException("setMemCacheSizePercent - percent must be "
                        + "between 0.05 and 0.8 (inclusive) - using default instead"));
            	return; //memCache already set as memCacheSize = DEFAULT_MEM_CACHE_SIZE; 
            }
    ...
    public static File getDiskCacheDir(Context context, String uniqueName) {
        ...
        final String cachePath = SDCardHelper.getCacheDir(context).getPath();
...
//    }
    ...
    public static File getExternalCacheDir(Context context) {
    	return SDCardHelper.getCacheDir(context);
//        if (SdkBuildUtils.hasFroyo()) {
...
//        return new File(Environment.getExternalStorageDirectory().getPath() + cacheDir);
    ...
    public static long getUsableSpace(File path) {
    	if (DeviceInfo.hasAPI9()) //SdkBuildUtils.hasGingerbread())
